...
function SkillNode({ skill, idx, total, radius = 4 }) {
  ...
  return (
    <Float position={[x, y, z]} speed={2} rotationIntensity={0.5} floatIntensity={0.5}>
      <group ref={groupRef}>
        <Sphere args={[0.2, 16, 16]}>
          <MeshDistortMaterial color={getColor()} speed={2} distort={0.3} radius={1} metalness={0.8} roughness={0.2} />
        </Sphere>
        <Text
          position={[0, 0.4, 0]}
          fontSize={0.2}
...
function MouseLight() {
  ...
  useFrame(({ mouse, viewport }) => {
    if (lightRef.current) {
      vec.set((mouse.x * viewport.width) / 2, (mouse.y * viewport.height) / 2, 3)
      lightRef.current.position.lerp(vec, 0.1)
    }
  })

  return <pointLight ref={lightRef} position={[0, 0, 3]} intensity={1} color="#ffffff" />
}
    ...
    <>
      ...
      <group ref={sceneRef}>
        ...
        {skills.map((skill, idx) => (
          <SkillNode key={skill.name} skill={skill} idx={idx} total={skills.length} />
        ))}
      </group>
      ...
      <div className="container px-4 md:px-6">
        <div className="flex flex-col items-center justify-center space-y-4 text-center">
          <div className="space-y-2">
            ...
            <h2 className="text-3xl font-bold tracking-tighter sm:text-5xl">Skills Galaxy</h2>
        ...
        <div className="mt-8 h-[600px] w-full rounded-xl overflow-hidden border bg-[#050816]">
          <Canvas>
        ...
        <div className="mt-6 text-center text-sm text-muted-foreground">
          <p>Drag to rotate the galaxy and explore my technical skills in 3D space.</p>
          <p className="mt-2">
            Each color represents a different skill category: frontend, backend, database, devops, tools, and
            architecture.
          </p>
        </div>
      </div>
